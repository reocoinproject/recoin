...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " reo-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  reo-tx [options] <hex-tx> [commands]  Update hex-encoded reo transaction\n" +
            "or:     reo-tx [options] -create [commands]   Create hex-encoded reo transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded reo transaction
