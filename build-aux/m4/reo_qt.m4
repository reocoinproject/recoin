...
dnl Output: If qt version is auto, set reo_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$reo_qt_want_version" = xauto && test "x$reo_qt_force" != xyes; then
    if test "x$reo_enable_qt" != xno; then
      AC_MSG_WARN([$1; reo-qt frontend will not be built])
    fi
    reo_enable_qt=no
    reo_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "x$reo_enable_qt" != xno && test "x$reo_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build reo-qt GUI (default=auto)])],
    [
     reo_qt_want_version=$withval
     if test "x$reo_qt_want_version" = xyes; then
       reo_qt_force=yes
       reo_qt_want_version=auto
     fi
    ],
    [reo_qt_want_version=auto])
...
dnl Outputs: reo_enable_qt, reo_enable_qt_dbus, reo_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "x$reo_cv_static_qt" = xyes; then
  ...
  BITCOIN_QT_CHECK([
    reo_enable_qt=yes
    reo_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      reo_enable_qt_test=no
    fi
    reo_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      reo_enable_qt_dbus=yes
    fi
  ...
  if test x$reo_enable_qt = xyes; then
    AC_MSG_RESULT([$reo_enable_qt ($QT_LIB_PREFIX)])
  else
    AC_MSG_RESULT([$reo_enable_qt])
  fi
...
dnl Output: reo_cv_qt5=yes|no
AC_DEFUN([_BITCOIN_QT_CHECK_QT5],[
  AC_CACHE_CHECK(for Qt 5, reo_cv_qt5,[
  AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
      #include <QtCore/qconfig.h>
      #ifndef QT_VERSION
      #  include <QtCore/qglobal.h>
...
dnl Requires: INCLUDES must be populated as necessary.
dnl Output: reo_cv_qt58=yes|no
AC_DEFUN([_BITCOIN_QT_CHECK_QT58],[
  AC_CACHE_CHECK(for > Qt 5.7, reo_cv_qt58,[
  AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
      #include <QtCore/qconfig.h>
      #ifndef QT_VERSION
      #  include <QtCore/qglobal.h>
...
dnl Output: reo_cv_static_qt=yes|no
dnl Output: Defines QT_STATICPLUGIN if plugins are static.
AC_DEFUN([_BITCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, reo_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
...
AC_DEFUN([_BITCOIN_QT_FIND_STATIC_PLUGINS],[
    if test "x$qt_plugin_path" != x; then
     ...
     m4_ifdef([PKG_CHECK_MODULES],[
       if test x$reo_cv_qt58 = xno; then
       ...
       if test "x$TARGET_OS" = xwindows; then
         AC_CACHE_CHECK(for Qt >= 5.6, reo_cv_need_platformsupport,[
           AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
               #include <QtCore/qconfig.h>
               #ifndef QT_VERSION
               #  include <QtCore/qglobal.h>
           ...
           [reo_cv_need_platformsupport=yes],
           [reo_cv_need_platformsupport=no])
         ])
         if test "x$reo_cv_need_platformsupport" = xyes; then
           if test x$reo_cv_qt58 = xno; then
...
dnl Inputs: reo_qt_want_version (from --with-gui=). The version to check
dnl         first.
dnl Inputs: $1: If reo_qt_want_version is "auto", check for this version
dnl         first.
...
dnl Inputs: reo_qt_want_version (from --with-gui=). The version to use.
...
AC_DEFUN([_BITCOIN_QT_FIND_LIBS_WITHOUT_PKGCONFIG],[
  ...
  BITCOIN_QT_CHECK([
    if test "x$reo_qt_want_version" = xauto; then
